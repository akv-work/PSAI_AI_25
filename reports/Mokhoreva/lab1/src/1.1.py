...
class TwoLayerNet:
    def __init__(self, rng_seed=1):
        ...
        self.W_hidden = rng.uniform(-0.1, 0.1, size=(2, 2))
        self.b_hidden = rng.uniform(-0.1, 0.1, size=2)
        self.W_out = rng.uniform(-0.1, 0.1, size=2)
        self.b_out = float(rng.uniform(-0.1, 0.1))

    def forward(self, sample):
        z_hidden = sample @ self.W_hidden + self.b_hidden
        a_hidden = sigma(z_hidden)
        z_out = a_hidden @ self.W_out + self.b_out
        a_out = sigma(z_out)
        return z_hidden, a_hidden, z_out, a_out

    def _update_weights(self, sample, target):
        z_hidden, a_hidden, z_out, a_out = self.forward(sample)

        d_out = (a_out - target) * sigma_prime(a_out)
        d_hidden = (self.W_out * d_out) * sigma_prime(a_hidden)

        self.W_out -= LEARNING_RATE * a_hidden * d_out
        self.b_out -= LEARNING_RATE * d_out

        self.W_hidden -= LEARNING_RATE * np.outer(sample, d_hidden)
    ...
    def total_error(self, inputs, targets):
        err = 0.0
        for sample, target in zip(inputs, targets):
            _, _, _, a_out = self.forward(sample)
            err += 0.5 * (a_out - target) ** 2
        return float(err)

    def predict(self, sample):
        _, _, _, a_out = self.forward(to_unit(sample))
        return float(from_unit(a_out))
